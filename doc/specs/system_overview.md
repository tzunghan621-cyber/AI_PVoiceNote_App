...
| **Summarizer** | 週期性呼叫 Gemma 4 產生/更新摘要。**在獨立 task 執行，不阻塞轉錄主迴圈**；同時最多一個 pending summary（以 `_summarizing` flag 擋連發）。詳見 §4.2 | 累積的校正後逐字稿 | 會議重點 + Action Items + 決議（增量更新），格式見 [[data_schema#4. 摘要結果]] |
...
**併發模型（G6）：** Summarizer 跑在獨立 task（`asyncio.create_task` fire-and-forget），不阻塞轉錄主迴圈。以 `_summarizing` flag 擋連發 — 同時最多一個 pending summary（週期觸發本身 180s 間隔，配合 flag 足以防連發，不需額外 queue）。transcribe 主迴圈僅消費 audio_source，不等待 summary 完成。

**停止路徑的 summary 處理（G2/G3）：** 使用者按停止 → transition 進 `processing` 狀態。
- 若 pending 週期 summary 正跑：等 `pending_summary_wait_sec`（預設 60s）→ 超時則 cancel 該 summary，用最近一版週期 summary 當 final（加 `fallback_reason="pending_summary_timeout"`）
- 若無 pending：觸發 final summary 生成，最長等 `final_summary_timeout_sec`（預設 120s）
...
本 App 面對使用者錄音的即時場景，對「資料完整性」與「降級可用性」有三項系統級原則：

1. **資料保全優先**：任何已處理過的 segments 都必須落盤，即使 partial。寧可使用者拿到「部分逐字稿 + 部分摘要」的 aborted session，也不要整場丟失。
2. **降級而非崩潰**：Summarizer 失敗 → 降級為「無摘要版」（session 仍 ready，附 fallback_reason）；Transcribe 單個 chunk 失敗 → 該 chunk 空 segment 略過，不中斷整條 pipeline；Ollama 失聯 → 用最近可用週期 summary 當 final。
3. **明確告知使用者**：所有降級 / 異常路徑必須在 UI 以持久提示（≥30 秒 SnackBar 或對話框）告知使用者，不靜默吞例外。log 端必用 `logger.exception` 留完整 traceback。

這些原則套用於 [[#4. 串流處理 Pipeline]] 的所有階段。具體實作指引見 [[pipeline_lifecycle_architecture_20260416]]。
