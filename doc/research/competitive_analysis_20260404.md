...
2. **逐字稿區參考 Otter**：
   - 時間戳可點擊（未來若支援音訊回放）
   - 校正處高亮標記（我們已有此設計，見 [[ui_spec#2.4 會後編輯模式]]）
...
2. **會議重點區**：全文可編輯（如 [[ui_spec#2.4 會後編輯模式]] 所設計），參考 Granola 的 contenteditable 體驗
...
**建議強化方向：**
- 在 [[ui_spec#2.4 會後編輯模式]] 的逐字稿區，校正處可「一鍵」新增遺漏詞條（降低回饋門檻）
...
建議 Director 優先確認 [[ui_spec#2.4 會後編輯模式]] 的三欄佈局細部互動設計，這是使用者體驗的核心差異化。
